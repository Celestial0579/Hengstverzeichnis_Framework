...
| Weder `public/uploads` noch `plugins`, `config`, `storage/logs`, `storage/horses` — auch nicht als Elternverzeichnis | Ein Addon, das `storage` beansprucht, nähme `storage/logs` und die Pferdefotos mit |
