...
class DsgvoPortalTest extends FunctionalTestCase {
     ...
     * Die CAPTCHA-Aufgabe wird bewusst ausgeschrieben gestellt - dieser Test
     * löst sie deshalb wie ein Mensch über die Wortbedeutung.
     *
    ...
    private function captchaQuestion(HttpResponse $page): string {
    ...
    /** Löst die Aufgabe wie ein Mensch: über die Bedeutung der Zahlwörter. */
    private function solveCaptcha(HttpResponse $page): int {
     ...
     */
    private static function resetDsgvoRateLimit(): void {
        $db = \App\Database::getInstance();
        $db->exec("DELETE FROM login_attempts WHERE type IN ('dsgvo_attempt', 'dsgvo_request')");
    }
